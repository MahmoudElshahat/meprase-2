...
// ************************************************************
    ...
    public function postCheckout(
        ...
        // dd($request);
        $token = $request->input('checkout-token');
        if (! EcommerceHelper::isCartEnabled()) {
            abort(404);
        }
        ...
        $responseResult =  $this->createPaymentSession($amount);//creat payment session with geidea

        if($responseResult && isset($responseResult['session']['id'])){

            $sessionId = $responseResult['session']['id'];

           $dataToStore =  Payment::handle_payment_responce_data($responseResult);
        //    dd($responseResult,$dataToStore);
           Payment::create($dataToStore);
        ...
        ->httpResponse()
        ->setError()
        ->setMessage(__('Payment failed!3333'));
...
// **************************************************************************************
// **************************************************************************************
public function createPaymentSession($amount)//1
{       

    $amount = floatval(str_replace(',', '', $amount));
    // $amount = $numericValue / 360;
    // dd($amount);
    // $currenct =   
    ...
    $currency = config('services.giedea.currency');
    ...
    $orderRefrance = number_format(rand(9999,100000));
    $url = $baseUrl.'/session';
    $orderId = $orderRefrance;
    // dd($currency,$publicKey,$ApiPassword,$orderRefrance);
    // $amount = number_format($amount,2);
    // $currency = $currency;
    $time=now()->format('m/d/Y h:i:s A');
    $amountStr = str_replace(',', '', number_format($amount,2));
    ...
    $data = [   
        ...
        "language" => "EN",
        "callbackUrl" => route('giedea.callback'),
        "returnUrl" => route('giedea.callback'),
        
        // "callbackUrl" =>'https://demo.mepras.com/products',
        // "returnUrl" => 'https://demo.mepras.com/products',
        "customer" => [
    ...
    if($responseResult['responseCode'] == '000'){
...
// **************************************************************************************
// **************************************************************************************
public function GiedaPaymentCallBack(Request $request){

    // dd($request);
    $sessionId = $request->query('sessionId') ?? null;
    $statusCode = $request->query('responseCode') ?? null;
    $orderId = $request->query('orderId') ?? null;
    $responseMessage = $request->query('responseMessage') ?? null;
    
    $row = Payment::where('gatway_session_id',$sessionId)->first();
    if($statusCode == '000' || $responseMessage == 'Success'){

        $row->status = 'success';
        $row->gatway_status = $responseMessage;
        $row->response_code = $statusCode;
        $row->gatway_order_id =  $orderId;
        $row->save();
